...
export function studyCard(item, helpers) {
  ...
  return `
    <article class="study-card" data-item-id="${escapeHtml(item.id)}">
      <span class="badge ${badgeClassByKind[item.kind] || "green"}">${kindLabels[item.kind] || escapeHtml(item.kind)}</span>
      ...
      <div class="card-actions">
        ${item.review ? `<button class="ghost-btn tiny-action-btn" data-cycle-review="${escapeHtml(item.id)}">복습: ${escapeHtml(reviewStatusText(item))}</button>` : ""}
        <button class="ghost-btn tiny-action-btn" data-edit-item="${escapeHtml(item.id)}">수정</button>
        <button class="danger-btn tiny-action-btn" data-delete-item="${escapeHtml(item.id)}">삭제</button>
      </div>
    </article>
  `;
}
